...
<head>
	...
	<title>{{Auth::user()->name}} Crp Panel | SDS MIS</title>
	...
	<div class="navbar navbar-expand-md navbar-dark">
		<div class="navbar-brand">
			<a href="{{url('/')}}" class="text-light">
				<h3 class="m-0"><b>Crp Panel Menu</b></h3>
			</a>
		</div>
			...
			<div class="sidebar-content">
				...
				<div class="card card-sidebar-mobile">
					<ul class="nav nav-sidebar" data-nav-type="accordion">
							<!-- Main -->
						<li class="nav-item-header"><div class="text-uppercase font-size-xs line-height-xs">Crp Panel</div> <i class="icon-menu" title="Main"></i></li>
						<li class="nav-item">
							<a href="{{route('crp.dashboard.index')}}" class="nav-link {{Request::is('crp/dashboard')?'active':''}}">
								<i class="icon-home4"></i>
								<span>Dashboard</span>
							</a>
						</li>	
						<li class="nav-item nav-item-submenu {{Request::is('crp/farming_profile*') || Request::is('crp/monthly_farming_report*') ?'nav-item-open':''}}">
							<a href="#" class="nav-link"><i class="icon-home4"></i> <span>Fishery Project</span></a>

							<ul class="nav nav-group-sub" data-submenu-title="Layouts" style="{{Request::is('crp/farming_profile*')  || Request::is('crp/monthly_farming_report*') ?'display:block':''}}">
								<li class="nav-item"><a href="{{route('crp.farming_profile.index')}}" class="nav-link {{Request::is('crp/farming_profile') || Request::is('crp/farming_profile/*') ?'active':''}}">Farming Profile</a></li>
								<li class="nav-item"><a href="{{route('crp.monthly_farming_report.index')}}" class="nav-link {{Request::is('crp/monthly_farming_report') || Request::is('crp/monthly_farming_report/*') ?'active':''}}">Monthly Farming Report</a></li>
							</ul>
						</li>
						<li class="nav-item">
						<a href="{{route('crp.training_report.index')}}" class="nav-link {{Request::is('crp/training_report') || Request::is('crp/training_report/*') ?'active':''}}">
						<i class="icon-home4"></i>
						...
						<li class="nav-item">
							<a href="{{route('crp.respondent_master.index')}}" class="nav-link {{Request::is('crp/respondent_master') || Request::is('crp/respondent_master/*')?'active':''}}">
								<i class="icon-home4"></i>
